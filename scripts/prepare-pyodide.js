...
async function downloadPackages() {
	...
	const pyodideVersion = await getExpectedPyodideVersion();
...
async function getExpectedPyodideVersion() {
	try {
		const installedPackageJson = JSON.parse(await readFile('node_modules/pyodide/package.json'));
		return installedPackageJson.version.replace('^', '');
	} catch {
		const packageJson = JSON.parse(await readFile('package.json'));
		return packageJson.dependencies.pyodide.replace('^', '');
	}
}
